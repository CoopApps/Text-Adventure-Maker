...
impl DaadCodeGenerator {
    ...
    pub fn generate(game: &DaadGame) -> String {
        Self::generate_for_language(game, game.default_language)
    }

    /// Generate DAAD source code for a specific language
    pub fn generate_for_language(game: &DaadGame, language: Language) -> String {
        let mut code = String::new();

        // Header with comments and #define statements
        code.push_str(&Self::generate_header(game, language));
        ...
        // 2. /VOC - Vocabulary (with translations for this language)
        code.push_str(&Self::generate_vocabulary_for_language(&game.vocabulary, language));
    ...
    fn generate_header(game: &DaadGame, language: Language) -> String {
    ...
    fn generate_vocabulary_for_language(vocab: &[VocabEntry], language: Language) -> String {
        let mut code = String::from(
            ";------------------------------------------------------------------------------\n\
             /VOC    ;Vocabulary\n\n"
        );

        // Helper to get word in target language
        let get_word = |entry: &VocabEntry| -> String {
            entry.translations
                .get(&language)
                .cloned()
